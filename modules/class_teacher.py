...
def app():
    ...
    with tab1:
        ...
        with col_filt2:
            show_all = st.checkbox("Show All Subjects", value=False, help="Enable to enter marks for subjects not assigned to you.")
        
        conn = database.get_connection()
        
        if show_all:
            # Fetch all subjects
            subjs_df = pd.read_sql("SELECT id, name FROM subjects ORDER BY name", conn)
        else:
        ...
        if subjs_df.empty:
            if show_all:
                 st.warning("No subjects found in system.")
            else:
                 st.warning("No subjects assigned to you for this Class. Check 'Show All Subjects' to edit others.")
        else:
